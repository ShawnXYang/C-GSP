...
if args.is_nips:
    print('is_nips')
    test_set = fix_labels_nips(args, test_set, pytorch=True, target_flag=True)
else:
...
# save
target_test_size = 0.
for i, (img, target_label) in enumerate(test_loader):
    save_name = os.path.basename(test_set.samples[i][0])
    target_label_numpy = target_label.numpy()[0]

    if target_label_numpy not in label_set: continue
    
    img, target_label = img.cuda(), target_label.cuda()
    target_test_size += img.size(0)

    target_tensor = torch.LongTensor(img.size(0))
    target_tensor.fill_(target_label_numpy)
    ...
    for idx in range(len(save_imgs)):
        g_img = transforms.ToPILImage('RGB')(save_imgs[idx])
        output_dir = 'results/gan_all/images'
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        g_img.save(os.path.join(output_dir, 't{}_{}'.format(target_label_numpy, save_name)))        
    
print('target_test_size:{}'.format(target_test_size)) 
